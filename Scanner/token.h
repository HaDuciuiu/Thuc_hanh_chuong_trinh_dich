...
typedef struct {
  char string[MAX_IDENT_LEN + 1];// mot chuoi dinh danh co do dai toi da la 15
  int lineNo, colNo;//dong hien tai , cot hien tai
  TokenType tokenType;// kieu token
  int value;// gia tri
} Token;

TokenType checkKeyword(char *string);// kiem tra string co thuoc loai keyword nao ko
