...
extern int lineNo;// dong hien tai
extern int colNo;// cot hien tai
extern int currentChar;

extern CharCode charCodes[];// mang charcode
...
void skipComment()
{
  while (1)
  {
    ...
    if (currentChar == -1) // khong doc duoc ki tu gi nua -- ket thuc file
    {                                         // End of File
...
Token *readIdentKeyword(void) // ham doc tu khoa dinh danh
{
  int count = 0;
  Token *token = makeToken(TK_IDENT, lineNo, colNo);// tao moi 1 token voi kieu va vi tri

  while (charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_DIGIT)// khi ki tu hien tai la chu cai hay chu so
  {
...
Token *readNumber(void)// doc so
{
  ...
  token->value = atoi(token->string); // atoi : ham chuyen tu 1 chuoi so (string) sang so nguyen

  return token;
}

Token *readConstChar(void)// doc vao hang ki tu
{
...
int scan(char *fileName) // ham chay scanner
{
  Token *token;
